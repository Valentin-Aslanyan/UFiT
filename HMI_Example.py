...
target_CR=2066	#Carrington rotation, here 25/Feb/2008
...
HMI_filename=get_HMI_magnetogram(2112,UFiT_home_directory)
HMI_fits=fits.open(HMI_filename)

Br_in=UFiT_PFSS.resample(HMI_fits[0].data,(num_t,num_p))
Br_in[np.isnan(Br_in)]=0.0

rg,rc,sg,sc,pg,pc,bg,bc=UFiT_PFSS.pfss(Br_in,num_r,Rss)
grid_r=np.exp(rg)
grid_t=np.arccos(sg)
grid_p=pg
B=bg.transpose(3,2,1,0)
