...
This package is intended to trace magnetic field lines and calculate the squashing factor $Q$. It is intended to read the outputs from a number of astrophysical codes, currently: \texttt{ARMS}, \texttt{Lare3d} and \texttt{DUMFRIC}; it saves the result in a self-contained manner. Also included is a related tool \texttt{USlip} to calculate the ``slippage rate'' of fieldlines, such as due to nonideal effects.
...
\newpage
{\large \textbf{Example 3: (\texttt{Python} call) Fieldline slippage for a Z-pinch}}

Open the script \texttt{Gaussian\_Current\_USlip.py} for editing. As in the example above, you should change the home directory and run the script by executing:

$\quad\quad$\texttt{python Gaussian\_Current\_USlip.py}
...
(Optional) If you have the \texttt{Python} library \texttt{mayavi} installed, you may set the variable \texttt{visualize\_3D=True} and re-run the script. This will produce the following figure:

\begin{center}
\includegraphics[scale=0.4]{Gaussian_Current_USlip3D.png}
\end{center}

The flat planes show $|\vec{\Sigma}|^2$ similarly to the figure above, and \texttt{UFiT} has been called to generate the helical fieldlines mentioned. Note that the cylindrical plane has not been correctly ``wrapped'', for visualization purposes. The appropriate array could be extended by copying the values $\vec{\Sigma}(\phi=0)\rightarrow \vec{\Sigma}(\phi=2\pi)$. Nontheless, the underlying calculation used the \texttt{Periodic\_PHI} condtion and correctly assumed this; this is merely a visual artifact.
...
You will need to add some command-line arguments from the table below:
...
\begin{tabular}{|c|l|}		
		...
		\texttt{-ue} & \textbf{u}ser \textbf{e}ndpoints (when you \textbf{only} want to define how to handle endpoints)  \\\hline
		...
		\texttt{-is} & \textbf{i}ntegration \textbf{s}cheme: $1 =$ 1$^{\mathrm{st}}$-order; $4 =$ RK4; \\\hline
...
\begin{tabular}{|c|l|}		
		...
		\texttt{SC:} & \textbf{s}ave \textbf{c}onnectivity type  \\\hline
		\texttt{UD:} & \textbf{u}ser \textbf{d}efined calculation  \\\hline
		\texttt{UE:} & \textbf{u}ser \textbf{d}ndpoints  \\\hline
		\texttt{NB:} & normalized $B$ for $Q$ calculation; \textbf{this is highly recommended} \\\hline
		\texttt{DL:} & step size \\\hline
		\texttt{MS:} & \textbf{m}ax \textbf{s}teps  \\\hline
		\texttt{IS:} & \textbf{i}ntegration \textbf{s}cheme: $1 =$ 1$^{\mathrm{st}}$-order; $4 =$ RK4; \\\hline
...
We define a ``regular'' 3-dimensional grid of points as one made up of an outer product of 3 individual vectors, $\vec{v}_1 \otimes \vec{v}_2 \otimes \vec{v}_3$, such that each vector gives points in its respective dimension and the set of grid points is every possible combination of elements from the three vectors. In other words, this is the ordinary bunch of cuboids you think of when a 3-dimensional grid comes to mind. Note that the spacing between the points does not need to be constant, or identical, and nor does the number of points in each dimension need to be identical. This is the preferred way to input the magnetic field to \texttt{UFiT}.

{\large \textbf{Separate (or staggered) grids}}

It is normally assumed that the $B$-field is defined on a single 3-dimensional grid, namely that at every grid point a vector $\vec{B}$ is completely known. A ``separate'' grid is one where each of the components of $B$ is defined at differing spatial locations. There are effectively three separate grids, which may have different numbers and separations of points. This means that the procedure for the interpolation of $B$ must be performed separately for each component.
...
which by default contains five subroutines:
\begin{enumerate}
...
\item\texttt{fieldline\_classification\_user} - specify how the type of fieldline, \textit{e.g.} open vs closed, should be classified
...
\item\texttt{trace\_cylindrical\_user} - as above, for cylindricals
\end{enumerate}
...
{\large \textbf{User defined endpoints}}

The type of fieldline is saved directly when running with the \texttt{--save\_connection} or \texttt{-sc} option, and indirectly by the sign of $Q$ when that is output. 
...
Description coming soon!$^{\mathrm{TM}}$


\newpage
{\Large \textbf{\texttt{USlip}}}

This tool is automatically compiled by \texttt{make}, exactly the same way as the rest of \texttt{UFiT}. You can run it from command line by executing:

$\quad\quad$\texttt{./Run\_USlip}
...
\textbf{Note:} you must provide the appropriate type of slash at the end of the output directory, as the name of the new output file will be appended directly at the end of whatever you put. For instance, 

$\quad\quad$\texttt{./Run\_USlip -b abc.xyz -od ./mydir/}

will produce a new file in a new directory, such that its path is \texttt{./mydir/abc.xyz}, while 

$\quad\quad$\texttt{./Run\_USlip -b abc.xyz -od ./new}

will produce a file called \texttt{newabc.xyz} in the working directory. This is a feature, not a bug! You may, for example, want to label the new files with a prefix; then, simply do not terminate your output directory with a slash.

\textbf{Important!} Do not overwrite the original file, for example by setting the output directory to be where the input files are stored.
